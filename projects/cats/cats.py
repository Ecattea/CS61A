...
def pick(paragraphs: list[str], select, k: int) -> str:
    ...
    """
    # BEGIN PROBLEM 1
    data = [x for x in paragraphs if select(x)]
    if k < len(data):
        return data[k]
    else:
        return ''
...
def about(keywords: list[str]):
    ...
    # BEGIN PROBLEM 2
    def f(paragraph):
        data = split(remove_punctuation(lower(paragraph)))
        contain = [x for x in keywords if x in data]
        #print(contain)
        if contain != []:
            return True
        else:
...
def accuracy(typed: str, source: str) -> float:
    ...
    # END PROBLEM 3
    # Better solution
    data = [1 for i, x in enumerate(typed_words) if i < len(source_words) and x == source_words[i]]
    return (len(data) / len(typed_words)) * 100
