...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    diff = abs(player_score % 10 - opponent_score // 10 % 10) * 3
    if diff == 0:
        return 1
    else: return diff
    # END PROBLEM 2
...
def sus_points(score):
    ...
    if num_factors(score) in (3, 4):
        while not is_prime(score) and score != 1:
            score += 1
    return score
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    start_num = 1
    result = 1
    avg1 = make_averaged(roll_dice, times_called)(start_num, dice)
    best_avg = avg1
    while start_num < 10:
        start_num += 1
        avg2 = make_averaged(roll_dice, times_called)(start_num, dice)
        if avg2 > best_avg:
            best_avg = avg2
            result = start_num
        avg1 = avg2
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    diff = sus_update(0, score, opponent_score) - score
    if diff >= threshold:
        return 0
    else: return num_rolls
    # END PROBLEM 11
